...
function Movement.TweenTo(targetCFrame, speed, callback)
    if Movement.IsTweening then Movement.StopTween() end
    ...
    local distance = Utils.Distance(hrp.Position, targetCFrame.Position)
    if distance < 15 then
    ...
    local tween = TweenService:Create(hrp, tweenInfo, {CFrame = targetCFrame * CFrame.new(0, 10, 0)})
...
function Movement.TeleportSafe(targetCFrame)
    task.wait(0.1 + math.random() * 0.1)
    Movement.Teleport(targetCFrame)
    task.wait(0.05)
end
...
function Movement.Fly(enable)
    ...
    if enable then
        ...
        local flyConnection = RunService.RenderStepped:Connect(function()
            ...
            local speed = getgenv().Config.FlySpeed or 150
            local cam = Camera
...
function Combat.EquipWeapon(weaponName)
    ...
    if equipped and equipped.Name:find(weaponName) then
        return true
    end
    ...
    for _, tool in pairs(backpack:GetChildren()) do
        if tool:IsA("Tool") then
            if tool.Name:find(weaponName) or tool.ToolTip:find(weaponName) then
                hum:EquipTool(tool)
                task.wait(0.1)
                return true
            end
...
function Combat.Attack(target)
    pcall(function()
        ...
        local now = tick()
        if now - Combat.LastAttack < 0.1 then return end
        ...
        if getgenv().Config.FastAttack then
            RemoteHandler.Fire("CommF_", "weaponL1", target, hrp)
        else
...
function Combat.UseSkill(skillKey)
    pcall(function()
        ...
        if now - lastUse < 0.5 then return end
...
function Combat.BringMobs(mobName, radius)
    pcall(function()
        ...
        for _, enemy in pairs(enemies:GetChildren()) do
            if enemy.Name == mobName or (not mobName) then
...
function Combat.ExpandHitbox(mobName, size)
    pcall(function()
        if not getgenv().Config.ExpandHitbox then return end
...
function AutoFarm.FarmMob(target, questData)
    ...
    if distance > 150 then
        Movement.TweenTo(targetHRP.CFrame * CFrame.new(0, 15, 0))
    else
        Movement.StopTween()
        ...
        Combat.EquipWeapon(getgenv().Config.SelectedWeapon or "Combat")
        Combat.Attack(target)
        Combat.SpamSkills()
    end
end

function AutoFarm.SafeModeCheck()
    if not getgenv().Config.SafeMode then return false end
    ...
    if healthPercent < (getgenv().Config.SafeHealthPercent or 30) then
        local hrp = Utils.GetHRP()
        if hrp then
...
function AutoFarm.Start()
    ...
    Utils.Notify("Auto Farm", "Started!", 2)
    ...
    local farmLoop = RunService.Heartbeat:Connect(function()
        local success, err = pcall(function()
            ...
            if AutoFarm.SafeModeCheck() then
                task.wait(1)
                return
            end
            ...
            if target then
                AutoFarm.NoMobCounter = 0
                AutoFarm.FarmMob(target, questData)
            else
                ...
                if AutoFarm.NoMobCounter > 500 and getgenv().Config.ServerHopNoMobs then
                    AutoFarm.NoMobCounter = 0
                    pcall(function()
        ...
        if not success then
            Utils.Log("AutoFarm Error: " .. tostring(err), "ERROR")
        end
    end)
...
function BossFarm.CollectDrops()
    if not getgenv().Config.AutoCollectDrops then return end
    
    pcall(function()
        local hrp = Utils.GetHRP()
        if not hrp then return end
        
        local drops = Workspace:FindFirstChild("Drops") or Workspace:FindFirstChild("GroundItems")
        if drops then
            for _, item in pairs(drops:GetChildren()) do
                local handle = item:FindFirstChild("Handle")
                if handle and Utils.Distance(hrp.Position, handle.Position) < 50 then
...
function BossFarm.Start()
    ...
    local bossLoop = RunService.Heartbeat:Connect(function()
        local success, err = pcall(function()
            ...
            local boss, bossData = BossFarm.GetBoss(bossName)
            
            if boss then
                BossFarm.CurrentBoss = boss.Name
                ...
                if bossHRP and hrp then
                    ...
                    if distance > 100 then
                        Movement.TweenTo(bossHRP.CFrame * CFrame.new(0, 25, 0))
                    else
                        Movement.StopTween()
